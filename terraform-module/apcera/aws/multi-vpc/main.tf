...
variable "DMZ-us-west-2_aws_ssh_key" {
  default = "eng-us-west-2"
}

variable "DMZ-us-west-2_aws_base_ami" {
...
variable "PRIVATE-us-west-2_aws_ssh_key" {
  default = "eng-us-west-2"
}

variable "PRIVATE-us-west-2_aws_base_ami" {
  description = "AMI ID to use for hosts other than the orchestrator. We no longer provide a default AMI here. Use the ami-copy module to copy Apcera's AMI to the AWS account this cluster runs in to avoid dependency issues."
}

variable "MGMT-us-west-2_aws_ssh_key" {
  default = "eng-us-west-2"
}

variable "MGMT-us-west-2_aws_base_ami" {
  description = "AMI ID to use for hosts other than the orchestrator. We no longer provide a default AMI here. Use the ami-copy module to copy Apcera's AMI to the AWS account this cluster runs in to avoid dependency issues."
}
...
variable "admin_contact" {
  default = "runq.develop.apc"
}
variable "service_id" {
  default = "CORP_3583_runq-apc"
}
